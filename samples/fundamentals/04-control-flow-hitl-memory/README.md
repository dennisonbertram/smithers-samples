...
> A workflow fans out three tasks in parallel, pauses for a human approval gate, runs a gated task only after approval, then writes a named fact to the memory store that survives across independent workflow runs.

## TL;DR

Three tasks run in parallel (capped at two at a time). The workflow then halts and waits for a human to type `approve`. Only after that does the gated task run. A memory fact written in one run is still there in the next run — the store persists across process invocations, not just within a session.
...
`<Parallel maxConcurrency={2}>` with three children starts exactly two tasks concurrently; the third waits until a slot frees. An `<Approval>` node halts the run (exit code 3, status `waiting-approval`) until you call `approve`. The gated task is conditionally rendered via `ctx.outputMaybe` — it only gets scheduled after the approval decision is read back on resume. `createMemoryStore` writes a named fact to `_smithers_memory_facts`; a second full workflow run shows the row unchanged, with the same `created_at_ms`.
...
Three composable primitives: bounded parallel fan-out (`<Parallel maxConcurrency>`), a durable human-in-the-loop approval gate (`<Approval>` + `ctx.outputMaybe`), and a cross-run key/value memory store (`createMemoryStore`). Together they cover workflows that pause for human decisions, resume without re-running completed work, and carry state across independent process invocations.
...
- **Data-migration saga with manual sign-off.** Parallelize schema-validation tasks across database shards, pause for a DBA to confirm row counts look correct, then resume the migration. Completed tasks are not re-run on resume (Smithers reconciles persisted state), so you only pay for the work still left to do.
