...
Each of these cost real debugging time and was confirmed by running Smithers `0.24.2` on Bun.
They're the highest-value things to know before you build.

## Setup

- **Use `zod@^4`.** Smithers introspects schemas via Zod v4 internals; a v3 schema fails with the
  cryptic `undefined is not an object (evaluating 'schema._zod.def')`.
- **No `react` dependency needed.** On 0.24 the minimal dep set is just
...
- **Model id: `claude-haiku-4-5` (not `claude-fable-5`).** The wrong model surfaces as a misleading
  *"the agent did not return valid JSON for the declared output schema"* error rather than a
  model-not-found error — you can burn many retries chasing a non-existent schema bug.
...
- **`AnthropicAgent` does native structured output by default.** On 0.24.2 this became *opt-in* for
  the `OpenAIAgent` / `CodexAgent` / `HermesAgent` (so agentic tool use keeps working) — pass
  `nativeStructuredOutput: true` for pure structured output on those. `AnthropicAgent` is unchanged.
...
- **`SagaStep` is type-only at the package root** — importing `<SagaStep>` throws
  *"Export named 'SagaStep' not found"*. Use the `steps` prop:
  ...
  To prove compensation, make a later step's `action` throw with `retries={0}`; earlier steps'
  `compensation` tasks run, and `onFailure="compensate"` lets the **run still finish** — so assert
